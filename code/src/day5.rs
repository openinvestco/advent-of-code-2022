pub mod day5 {
    pub mod part_1 {
        ...
        fn grabBox(line: Vec<char>, idx: usize, row: &mut Vec<char>) {
        ...
        fn storeBlocks(line: Vec<char>) -> Vec<char> {
            let row: &mut Vec<char> = &mut Vec::new();
            grabBox(line, 0, row);
            return row.to_vec();
        }

        fn moveBlocks(numTimes: u32, from: &RefCell<Vec<char>>, to: &RefCell<Vec<char>>) {
            for _ in 0..numTimes {
        ...
        fn constructStacks(blocks: &mut Vec<RefCell<Vec<char>>>, stacks: &mut Vec<RefCell<Vec<char>>>) {
